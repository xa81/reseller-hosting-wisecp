...
| 6 | ~~Ayrıştırma hatası yolunda redaksiyon atlanıyor~~ **Kapandı.** `DNAHosting_Http::safeSummary($action, $body)` eklendi: özeti önce **redakte**, sonra **maskelenmiş** metinden çıkarır. `Cpanel::unwrap()` ve `Plesk::attempt()` artık ham gövde yerine bunu kullanıyor. Static `summarise()` yalnızca sır maskesini değil **redaksiyonu da** atlıyordu; ikisi birden kapandı. Plesk'e ayrıca gövde şeklinden bağımsız `PLESKSESSID=...` deseni eklendi — `<id>` yalnızca düzgün XML'de tutuyordu, bozuk gövdede **log da** korumasızdı. | Doğrulandı (7 test; redaksiyonu ve deseni tek tek geri alarak testlerin gerçekten düştüğü görüldü) |
| 7 | ~~Admin "Sipariş Otomasyonu" kaydı `options['config']`'i siliyor~~ **Kapandı.** `ServerModule::edit_order_params()` `config`/`creation_info`'yu POST'tan `(int)` olarak okuyor (`Modules.php:421-422`) ve sonunda `config[$this->entity_id_name]` yoksa `config` + `established`'i **siliyor** (`Modules.php:445`). Varsayılan anahtar `"vm_id"`; bizimki `config.user` (çekirdek `helpers/orders.php:2849`'da yazıyor). Düzeltme iki parçalı: `public $entity_id_name = 'user'` + `save_adminArea_service_fields()` — ikincisi çekirdeğin `(int)` cast'inin üstüne mevcut değerleri geri koyar (`Modules.php:423-435`). ApisCP / KeyHelp / ISPmanager aynı kancayı tanımlıyor; CentOSWebPanel, CyberPanel, HestiaCP, VestaCP tanımlamıyor (aynı açığa sahipler). `entity_id_name`'in çekirdekte başka kullanımı yok (tek yer: `Modules.php:445`), bu yüzden değiştirmek yan etkisiz. | Çekirdek algoritması birebir taklit edilerek doğrulandı (önce: `config` silinip `panelUser()` → `error-no-order`; sonra: korunuyor). **Canlıda teyit edilmeli** — kaynak ionCube'dan çözülmüş koddur. |
